...
//  RyukSign
//
//  A browser for RyukSign's own Documents folder: every file the app stores, including the
	...
	private var _summary: some View {
		Section {
			...
			if let total = _storage.report?.total {
				LabeledContent(.localized("RyukSign storage"), value: total.formattedFileSize)
			}
			...
			Text(.localized("Everything RyukSign keeps on this device lives here. Deleting an app folder removes its Library entry as well."))
		}
	}
